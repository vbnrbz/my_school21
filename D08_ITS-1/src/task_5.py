...
from src.utils import add_patient_to_schedule, add_tech_times_to_schedule
import csv
...
''')
        ...
        if quest == '1':
            ...
            tech_times= input('Enter technical times separated by commas and spaces, they should be in the schedule (08:00, 09:30): ').split(', ')
            f = add_tech_times_to_schedule(*tech_times)
            with open('src/schedule.csv', encoding='utf-8', newline='') as file:
                csv_reader = list(csv.reader(file, delimiter=','))
                print(tabulate(csv_reader, headers='firstrow', tablefmt='github'))
